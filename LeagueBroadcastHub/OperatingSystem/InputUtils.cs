...
namespace LeagueBroadcastHub.OperatingSystem
{
    class InputUtils
    {
        ...
        private static Input[] InputStart = new Input[]
        {
            new Input
            {
                type = (int)Inputtype.Keyboard,
        ...
        private static Input[] zoom = new Input[]
        {
            new Input
            {
        ...
        public static void SendAToLeague()
        {
            SendToLeague(InputA);
        }

        public static async void InitLeague()
        {
            if (GameController.LeagueProcess == null)
                return;
            Process p = GetActiveProcess();
            Point cursorP = GetCursorPosition();
            SetForegroundWindow(GameController.LeagueProcess.MainWindowHandle);
            await Task.Delay(50);
            SendInput((uint)InputStart.Length, InputStart, Marshal.SizeOf(typeof(Input)));
            
            /* Zoom out... not working for now
            SendKeyDown(KeyCode.LCONTROL);
            //SendKeyDown(KeyCode.LSHIFT);
            await Task.Delay(10);
            SendKeyPress(KeyCode.KEY_Z);
            SendKeyPress(KeyCode.KEY_Y);
            await Task.Delay(10);
            //SendKeyUp(KeyCode.LSHIFT);
            SendKeyUp(KeyCode.LCONTROL);
            SendInput((uint)zoom.Length, zoom, Marshal.SizeOf(typeof(Input)));
            */
            await Task.Delay(50);
        ...
        private static async void SendToLeague(Input[] Input)
        {
            if (GameController.LeagueProcess == null)
                return;
            Process p = GetActiveProcess();
